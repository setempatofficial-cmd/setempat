// components/UploadModal.jsx - FINAL VERSION dengan Cache & Debounce
"use client";
import { useState, useEffect, useMemo, useCallback, useRef } from "react";
import { createPortal } from "react-dom";
import { useRouter } from "next/navigation";
import { motion, AnimatePresence } from "framer-motion";
import { X, Search, MapPin, Camera, Loader2, Send, Link as LinkIcon, CheckCircle } from "lucide-react";
import { supabase } from "@/lib/supabaseClient";
import { CldUploadWidget } from "next-cloudinary";

export default function UploadModal({ isOpen, onClose, userId, userRole }) {
  const router = useRouter();
  const [allTempatCache, setAllTempatCache] = useState([]); // CACHE semua data
  ...
  // Load semua tempat SEKALI SAJA saat modal pertama kali dibuka
  const loadAllTempat = useCallback(async () => {
    // Jika sudah ada cache, skip fetch
    if (allTempatCache.length > 0) {
      console.log("Using cached data:", allTempatCache.length);
      return;
    }
    
    setLoading(true);
    try {
      console.log("Fetching all places from feed_view...");
      const { data, error } = await supabase
        .from("feed_view")
        .select("id, name, category, alamat")
        .limit(200); // Ambil lebih banyak sekaligus
      
      if (error) throw error;
      
      console.log(`Loaded ${data?.length || 0} places`);
      setAllTempatCache(data || []);
      setTempatList(data || []);
    } catch (err) {
  ...
  }, [allTempatCache.length]);

  // Panggil loadAllTempat saat modal buka (hanya sekali)
  useEffect(() => {
    if (isOpen && allTempatCache.length === 0) {
      loadAllTempat();
    }
  }, [isOpen, loadAllTempat, allTempatCache.length]);

  // Filter tempat di FRONTEND (SUPER CEPAT) dengan debounce
  useEffect(() => {
    ...
    // Debounce 200ms untuk pengalaman mengetik yang smooth
    debounceTimerRef.current = setTimeout(() => {
      if (!searchQuery.trim()) {
        setTempatList(allTempatCache);
        return;
      }
      
      const term = searchQuery.trim().toLowerCase();
      const filtered = allTempatCache.filter(item => 
        item.name?.toLowerCase().includes(term) ||
        item.category?.toLowerCase().includes(term) ||
        item.alamat?.toLowerCase().includes(term)
      );
      
      setTempatList(filtered);
    }, 200);
  ...
  }, [searchQuery, allTempatCache]);

  // Hasil pencarian yang ditampilkan (langsung dari tempatList yang sudah difilter)
  const displayTempat = useMemo(() => {
    if (!searchQuery.trim()) return [];
    return tempatList;
  }, [searchQuery, tempatList]);
  ...
  const handleUploadToSupabase = async (url, type) => {
    ...
    try {
      ...
      setSuccessMessage(`✅ Berhasil upload ke ${selectedTempat.name}`);
      setShowSuccess(true);
  ...
  return createPortal(
    <div className="fixed inset-0 z-[9999] flex items-end sm:items-center justify-center p-4 bg-black/70 backdrop-blur-md">
      <div className="w-full max-w-md bg-zinc-900 rounded-t-2xl sm:rounded-2xl overflow-hidden border border-white/10">
        {/* Header */}
        ...
        <div className="p-4 max-h-[70vh] overflow-y-auto space-y-4">
          ...
          {/* PENCARIAN TEMPAT - DENGAN INDIKATOR LOADING */}
          <div>
            ...
            {searchQuery && (
              <div className="mt-2 max-h-48 overflow-y-auto space-y-1">
                ...
                ) : displayTempat.length === 0 ? (
                  <div className="p-4 text-center text-white/40 text-sm">
                    Tidak ada tempat ditemukan
                  </div>
                ) : (
                  displayTempat.map((tempat) => (
                    <button
                      key={tempat.id}
                      onClick={() => {
        ...
        <AnimatePresence>
          {showSuccess && (
            ...
            >
              <div className="bg-emerald-500 rounded-xl p-4 flex items-center gap-3 shadow-xl">
                <CheckCircle size={24} className="text-white" />
                <div>
                  <p className="text-white font-semibold text-sm">Upload Berhasil!</p>
