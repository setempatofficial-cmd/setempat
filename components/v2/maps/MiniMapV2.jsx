// components/v2/maps/MiniMapV2.jsx
'use client';

import { useEffect, useState } from 'react';
import dynamic from 'next/dynamic';

// ✅ Import dinamis untuk semua komponen Leaflet
...
const LoadingFallback = () => (
  <div className="h-full w-full bg-zinc-800/50 animate-pulse flex items-center justify-center rounded-2xl">
    <div className="flex flex-col items-center gap-2">
      <div className="w-6 h-6 border-2 border-cyan-500/30 border-t-cyan-500 rounded-full animate-spin" />
      <p className="text-[10px] text-gray-400">Memuat peta...</p>
    </div>
  </div>
);
...
export default function MiniMapV2({ lat, lng, theme, radius = 1, showRadius = true, isInteractive = false }) {
  ...
  if (!lat || !lng) {
    return (
      <div className="relative w-full h-full bg-zinc-900 flex items-center justify-center rounded-2xl">
