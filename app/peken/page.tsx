...
export default function PekenPage() {
  ...
  useEffect(() => {
    const handleLocationUpdate = (event) => {
      const parts = event.detail?.placeName?.split(",").map(p => p.trim());
      if (parts?.[0]) setManualLocationName(parts[0]);
    };
  ...
  useEffect(() => {
    const handleRefreshLapak = () => {
      if (activeTab === 'lapakku') {
        window.dispatchEvent(new CustomEvent('refresh-lapak-request'));
      }
    };
    
    window.addEventListener('refresh-lapak', handleRefreshLapak);
  ...
  <OjekSection onBack={() => setActiveTab('beranda')} locationName={finalLocationName} />
        ...
        {activeTab === 'donasi' && (
  <DonasiSection onBack={() => setActiveTab('beranda')} locationName={finalLocationName} />
       )}
      ...
      <BottomNav 
        activeTab={activeTab} 
        setActiveTab={setActiveTab} 
        onPlusClick={() => toggleModal('upload', true)} 
      />
  ...
  />
      ...
      <FormPanyangan 
        ...
        onClose={() => { toggleModal('formPanyangan', false); setEditingProduct(null); }}
...
function PekenHome({ location, setActiveTab, onHomeClick }) {
  return (
    <div className="animate-in fade-in slide-in-from-bottom-4 duration-500 space-y-8">
      <div className="flex justify-between items-start mb-4">
        <div>
          <h1 className="text-2xl font-black text-slate-900 tracking-tighter uppercase">PEKEN</h1>
          <div className="flex items-center text-[10px] text-slate-400 font-bold uppercase tracking-[0.2em] mt-1">
            <MapPin size={10} className="mr-1 text-orange-500" /> {location}
          </div>
        </div>
        <div className="flex gap-2">
          ...
          <button onClick={onHomeClick} className="p-2.5 bg-slate-900 rounded-2xl text-white active:scale-95 transition-all">
            <Home size={18} />
          </button>
        </div>
      </div>
...
function MenuCard({ icon: Icon, title, desc, color, onClick }) {
  return (
    <button 
      onClick={onClick} 
      className="flex flex-col items-start p-5 bg-white border border-slate-100 rounded-[32px] text-left hover:border-orange-200 transition-all active:scale-95 group"
    >
      <div className={`p-3 rounded-2xl mb-3 transition-transform group-hover:scale-110 ${color}`}>
        <Icon size={22} />
      </div>
      <h4 className="font-black text-slate-800 text-xs uppercase tracking-tight">{title}</h4>
  ...
  return (
    <button 
      ...
      className={`flex flex-col items-center justify-center w-14 h-12 transition-all ${
        active ? 'text-orange-400' : 'text-slate-500 opacity-60'
      }`}
    >
      <div className="relative">
        <Icon size={18} strokeWidth={active ? 2.5 : 2} />
        {label === 'Bakul' && (
          <div className="absolute -top-1 -right-1 w-2 h-2 bg-red-500 rounded-full border-2 border-slate-900 animate-bounce" />
        )}
      </div>
