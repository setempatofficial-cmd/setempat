...
export default function AIModal({ isOpen, onClose, tempat }) {
  ...
  useEffect(() => {
    if (isOpen && tempat) {
      // Pesan pembuka yang lebih real-time friendly
      setMessages([
        {
          id: 1,
          type: "ai",
          text: `Halo! Saya warga AI Setempat yang ngerti **kondisi real-time** di ${tempat.name}.\n\nAdakah yang bisa dibantu?`,
          time: "Baru saja",
        },
      ]);
  ...
  const handleSend = () => {
    ...
    // Simulasi AI dengan respons real-time
    setTimeout(() => {
      const aiResponse = {
        id: Date.now() + 1,
        type: "ai",
        text: getRealTimeResponse(input, tempat),
        time: "Baru saja",
      };
  ...
  // Fungsi untuk memberikan respons real-time
  const getRealTimeResponse = (question, tempat) => {
