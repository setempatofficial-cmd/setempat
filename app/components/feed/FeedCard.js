...
// Cache untuk hasil processFeedItem
...
export default function FeedCard({
  ...
  useEffect(() => {
    ...
    const observer = new IntersectionObserver(
      ...
      { rootMargin: "100px" } // Mulai render 100px sebelum masuk viewport
    );
  ...
  // Optimasi processFeedItem dengan cache
  const feed = useMemo(() => {
    if (!item) return { allSignals: [], narasiCerita: "", distance: null };
    
    // Buat cache key
    const cacheKey = `${item.id}-${locationReady}-${location?.latitude || '0'}-${location?.longitude || '0'}`;
    ...
    // Proses dan simpan ke cache
    const result = processFeedItem({ item, comments, locationReady, location });
    feedCache.set(cacheKey, result);
    
    // Batasi ukuran cache (opsional)
  ...
  return (
    <div ref={cardRef} className="relative mb-6 w-full">
      {isVisible ? (
        <motion.div 
          initial={{ opacity: 0 }}
          animate={{ opacity: 1 }}
        ...
        >
          <div className={`relative overflow-hidden rounded-[40px] ${theme.card} border ${theme.border} shadow-2xl flex flex-col`}>
            {/* Konten card tetap sama persis dari sini ke bawah */}
            {/* --- 1. HEADER --- */}
        ...
        // Placeholder ringan saat card belum visible
        <div className={`relative overflow-hidden rounded-[40px] ${theme.card} border ${theme.border} shadow-2xl`} style={{ height: '600px' }}>
          <div className="w-full h-full bg-gradient-to-b from-transparent to-black/5 animate-pulse" />
        </div>
      )}
    </div>
  );
