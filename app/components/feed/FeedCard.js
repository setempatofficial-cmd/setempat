    ...
    <div className="group bg-white rounded-[32px] overflow-hidden border border-gray-100 shadow-sm hover:shadow-xl transition-all duration-500 mb-10 mx-2 relative">
      ...
      <div className="px-6 pt-5 pb-3 flex justify-between items-center">
        ...
        {feed.category && (
          <div className="px-3 py-1 bg-gray-50 rounded-lg border border-gray-100">
            <span className="text-[9px] font-black text-gray-400 uppercase tracking-tighter">{feed.category}</span>
          </div>
        )}
      </div>
        ...
        <div className={`relative overflow-hidden bg-gradient-to-br ${headerTheme} rounded-2xl p-5 shadow-md`}>
          <div className="relative flex items-center gap-4">
            ...
            <div className="flex-1">
              <h2 className="text-white text-[17px] font-bold leading-tight tracking-tight">{feed.headline?.text}</h2>
      ...
      <div className="px-4">
        <div className="rounded-2xl overflow-hidden shadow-lg group-hover:scale-[1.01] transition-transform duration-700">
          <PhotoSlider photos={photos} itemId={item.id} selectedPhotoIndex={selectedPhotoIndex?.[item.id] || 0} setSelectedPhotoIndex={setSelectedPhotoIndex} isRamai={feed?.isRamai} isViral={feed?.isViral} />
        </div>
      </div>
      ...
      <div className="px-6 mt-6">
        <div className="flex justify-between items-start mb-6">
          ...
          <div className={`px-3 py-2 rounded-xl border font-black text-[11px] uppercase tracking-tighter shadow-sm shrink-0 ${feed.badgeColor}`}>
            {feed.badgeStatus}
          </div>
        </div>

        {/* HORIZONTAL SIGNALS (DENGAN RED DOT ANIMATION) */}
          ...
          <div className="flex overflow-x-auto pb-6 px-6 gap-3 snap-x no-scrollbar">
              ...
              <div key={i} className="flex-shrink-0 w-[240px] snap-center bg-gray-50 rounded-2xl p-4 border border-gray-100 flex flex-col justify-between relative overflow-hidden">
                <div>
                  <div className="flex items-center gap-2 mb-3">
                    <img src={msg.avatar} className="w-8 h-8 rounded-lg bg-white border border-gray-100 shadow-sm" alt="user" />
                    <div className="flex flex-col">
                      <div className="flex items-center gap-1.5">
                        <span className="text-[11px] font-black text-gray-900 leading-none">@{msg.username}</span>
                        <div className="flex items-center gap-1 bg-white px-1.5 py-0.5 rounded-full border border-gray-100 shadow-sm">
                          {msg.isLive && (
                          ...
                          <span className="text-[9px] text-gray-400 font-bold leading-none">{msg.timeAgo}</span>
                        </div>
                      </div>
            ...
            <div onClick={() => openAIModal(item)} className="flex-shrink-0 w-[240px] snap-center bg-indigo-600 rounded-2xl p-5 shadow-lg cursor-pointer active:scale-95 transition-all">
              <div className="flex items-center gap-3 mb-3">
                ...
                <span className="text-[10px] font-black text-white uppercase tracking-widest text-[8px]">AI Check</span>
              </div>
      ...
      {/* 5. INTERACTION BAR (AKSI KANAN BAWAH) */}
      <div className="px-6 py-5 bg-gray-50/50 flex items-center justify-end border-t border-gray-100 gap-2">
          <button onClick={() => openKomentarModal(item)} className="flex items-center gap-2 px-5 py-3 bg-white rounded-xl shadow-sm border border-gray-100 active:scale-95 transition-all hover:bg-gray-50">
            ...
            <div className="text-left">
              <p className="text-[11px] font-black text-gray-900 leading-none uppercase tracking-tighter">Diskusi</p>
              <p className="text-[9px] text-gray-400 font-bold tracking-tighter mt-1">{(comments[item.id]?.length || 0)} Respon</p>
            </div>
          </button>
      ...
      {/* TOAST */}
      <div className={`fixed bottom-24 left-1/2 -translate-x-1/2 z-[100] transition-all duration-500 transform ${toast.show ? "translate-y-0 opacity-100" : "translate-y-12 opacity-0 pointer-events-none"}`}>
