...
<div className="px-4 pt-3 text-xs text-gray-500">
  ...
  <div className="flex items-center gap-2">
    ...
    {activityTime ? (
      <span>Terakhir terpantau {formatTimeAgo(activityTime)}</span>
    ) : (
      <span className="text-gray-400">Belum ada aktivitas terbaru</span>
    )}
  </div>
...
{item.distance < 0.05
  ? "Di sekitar Anda"
  : item.distance < 1
  ? `${Math.round(item.distance * 1000)}m dari Anda`
