...
export default function LiveInsight({ 
  ...
  const insights = useMemo(() => {
    const getSourceInfo = (item) => {
      ...
      if (isFromWarga || (!item.source_platform && item.user_name)) {
        ...
        return { icon: "👤", name, source: "warga", label: "WARGA" };
      }
  ...
  // Pengaman: fallback ke objek kosong jika insights[index] undefined
  ...
  return (
    <div className={`rounded-[32px] p-6 border transition-all duration-700 relative overflow-hidden shadow-xl backdrop-blur-xl ${
