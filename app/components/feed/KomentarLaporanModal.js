// app/components/feed/KomentarLaporanModal.js
"use client";
...
// ── OPTIMASI: Fetch komentar dengan SELECT spesifik ──────────────────────────────
async function fetchKomentarLaporan(laporanId) {
  if (!laporanId) return [];

  try {
    const { data: komentarData, error: komentarError } = await supabase
      .from("komentar_laporan")
      .select("id, laporan_id, parent_id, user_id, user_name, user_avatar, content, likes, created_at")
    ...
    console.error("Fetch komentar laporan error:", err);
    return [];
  }
}
...
// 🔥 FIX: Proper size mapping for Tailwind classes
const sizeMap = {
  7: "w-7 h-7",
  8: "w-8 h-8",
  9: "w-9 h-9"
      ...
      {hasReplies && (
        <div className={`${depth === 0 ? "ml-11" : "ml-9"}`}>
          <button
            ...
            aria-label={showReplies ? "Sembunyikan balasan" : `Lihat ${item.replies.length} balasan`}
          >
...
const sendNotificationToLaporanOwner = async (laporanId, content, currentUser, commentId) => {
  try {
    ...
    if (laporanData?.user_id && laporanData.user_id !== currentUser?.id) {
      await supabase.from("warung_info").insert({
        ...
        type: "komentar_laporan",
        title: "💬 Komentar Baru di Postinganmu",
        message: `${currentUser?.user_metadata?.full_name || currentUser?.email?.split("@")[0]} mengomentari postinganmu`,
        content: content.slice(0, 150),
        reference_id: commentId,
        reference_type: "komentar_laporan",
...
// ── MAIN COMPONENT ────────────────────────────────────────────────────────────
export default function KomentarLaporanModal({ isOpen, onClose, laporan, theme }) {
  const { user } = useAuth();
  ...
  // 🔥 OPTIMASI: Fetch komentar dengan cache
  useEffect(() => {
    if (!isOpen || !laporan?.id) return;

    const cacheKey = `komentar_${laporan.id}`;
    ...
    setLoading(true);
    fetchKomentarLaporan(laporan.id)
      .then(data => {
        setKomentar(data);
  ...
  // 🔥 FIX: Improved touch handlers to prevent scroll interference
  ...
  const handleSubmit = useCallback(async () => {
    ...
    try {
      const { data, error } = await supabase.from("komentar_laporan").insert([{
        laporan_id: laporan.id,
        parent_id: parentId,
        user_id: user.id,
        user_name: userName,
      ...
      if (!parentId) {
        await sendNotificationToLaporanOwner(laporan.id, text, user, data.id);
      }

      const newItem = { ...data, replies: [], is_verified: isUserVerified };
  ...
  }, [input, submitting, user, laporan?.id, replyTo, insertReply, replaceItem, removeItem]);
      ...
      >
        ...
        <div className="flex items-center justify-between px-4 py-3 border-b border-slate-100 flex-shrink-0">
          <div className="flex items-center gap-2">
            <span className="text-lg">💬</span>
            <div>
              <h2 className="text-[14px] font-bold text-slate-900">Jagongan Warga</h2>
              <p className="text-[11px] text-slate-400 line-clamp-1">{laporan?.deskripsi?.slice(0, 50)}...</p>
            </div>
          </div>
          <div className="flex items-center gap-2">
