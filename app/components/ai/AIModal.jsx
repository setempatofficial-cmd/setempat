...
export default function AIModal({
  ...
  // 🔥 CEK APAKAH MODE KENTONGAN
  const isKentonganMode = !!kentongan;
  const activeItem = item || tempat;
  const activeLocationName = locationName || tempat?.name || (isKentonganMode ? "Pengumuman Resmi" : "Sekitar");
  ...
  const fetchFreshLaporan = useCallback(async (tempatId) => {
    if (!tempatId || isKentonganMode) return;
    try {
  ...
  useEffect(() => {
    if (isOpen && activeItem?.id && !isKentonganMode) {
      fetchFreshLaporan(activeItem.id);
    }
  }, [isOpen, activeItem?.id, fetchFreshLaporan, isKentonganMode]);
  
  const quickActions = useMemo(() => {
    if (isKentonganMode) return [];
    const hasLaporan = (freshStats?.total || 0) > 0;
    return getQuickActionsByCategory(activeItem?.category, hasLaporan);
  }, [activeItem?.category, freshStats?.total, isKentonganMode]);
  
  // 🔥 OPENING MESSAGE UNTUK TEMPAT (yang sudah ada)
  useEffect(() => {
    if (!isOpen || isKentonganMode) return;
  ...
  }, [isOpen, activeItem?.id, freshReports, freshStats, jarak, getUniqueId, isKentonganMode]);
  
  // 🔥 OPENING MESSAGE UNTUK KENTONGAN (TAMBAHKAN INI)
  useEffect(() => {
    if (!isOpen || !isKentonganMode || !kentongan) return;
    if (hasInitialized.current) return;
...
💬 **Ada yang ingin kamu tanyakan tentang pengumuman ini?**`;
  ...
  }, [isOpen, isKentonganMode, kentongan, getUniqueId]);
  ...
  const handleSend = useCallback(async (customMessage) => {
    ...
    // 🔥 CEK LAPOR INTENT (hanya untuk mode tempat)
    if (!isKentonganMode && isLaporIntent(msg)) {
      setTimeout(() => {
        setIsTyping(false);
        setMessages(prev => [...prev, {
          id: getUniqueId(),
  ...
  }, [input, user, activeItem, freshReports, getUniqueId, isKentonganMode, kentongan]);
  ...
  return (
    <div className="fixed inset-0 z-[2000] flex items-end justify-center sm:items-center">
      ...
      >
        ...
        <div className="flex-1 overflow-y-auto px-4 py-4 space-y-4">
          {messages.map((msg) => (
            <AIMessageBubble
              ...
              showLaporButton={msg.showLaporButton && !isKentonganMode}
            />
          ))}
          {isTyping && (
        ...
        <AnimatePresence>
          {showLaporPanel && !isKentonganMode && (
            <LaporPanel
              mode="media"
              tempat={tempat}
        ...
        {!showLaporPanel && (
          <div className="flex-shrink-0 border-t border-gray-100 dark:border-white/10 px-3 py-3">
            ...
            <AIInput
              ...
              onSend={() => handleSend()}
              isTyping={isTyping}
              placeholder={isKentonganMode ? "Tanya tentang pengumuman ini..." : "Tanya kondisi, jam buka, atau langsung cerita..."}
            />
            
            <p className="text-[8px] text-center text-slate-400 mt-2">
              {isKentonganMode 
                ? "✨ Tanya detail pengumuman, dampak, atau instruksi lebih lanjut"
