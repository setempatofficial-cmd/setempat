...
export default function LaporPanel({
  ...
  // 🆕 STATE UNTUK NOMINATIM (JALAN / LOKASI UMUM)
  ...
  // Cek kategori untuk tempat yang dipilih
  ...
  // 🔥 FETCH TEMPAT DARI DATABASE (Cepat!)
  const fetchTempatList = useCallback(async (searchQuery = "") => {
    setIsLoadingTempat(true);
    try {
      let query = supabase
        .from("tempat") // Pake tabel asli, bukan feed_view
  ...
  // 🔍 SEARCH NOMINATIM (untuk jalan/lokasi umum)
  ...
  // 🔥 DEBOUNCE UNTUK PENCARIAN (Database + Nominatim)
  ...
  useEffect(() => {
    ...
    debounceTimer.current = setTimeout(() => {
      if (tempatQuery.length > 1 || tempatQuery === "") {
        ...
        // Cari di Nominatim juga (tapi beda flow)
        if (tempatQuery.length >= 3 && showNominatim) {
          searchNominatim(tempatQuery);
        } else {
          setNominatimResults([]);
        }
      }
  ...
  }, [tempatQuery, fetchTempatList, showNominatim]);
  ...
  // Pilih tempat (dari database atau nominatim)
  ...
  const isNomimatimLocation = activeTempat?.source === 'nominatim';
  ...
  const finalizeUpload = async () => {
    if (!activeTempat?.id && !isNomimatimLocation) {
      alert("Lokasi tidak valid. Silakan pilih lokasi terlebih dahulu.");
      setStep("pick_tempat");
      return;
    }

    if (!condition && !trafficCondition && !isNomimatimLocation) {
      alert("Pilih kondisi tempat terlebih dahulu");
      return;
    }

    if (isNomimatimLocation && !trafficCondition) {
    ...
    try {
      ...
      const tempatIdValue = !isNomimatimLocation && activeTempat?.id ? parseInt(activeTempat.id) : null;
      ...
      const reportData = {
        ...
        status: "approved",
        // 🆕 Kolom untuk lokasi dari Nominatim
        lokasi_name: isNomimatimLocation ? activeTempat.name : null,
        lokasi_lat: isNomimatimLocation ? activeTempat.latitude : null,
        lokasi_lng: isNomimatimLocation ? activeTempat.longitude : null,
        report_type: isNomimatimLocation ? 'general_location' : 'place',
      };
      ...
      toastNotif.innerHTML = `
        ...
        <span class="text-[10px] opacity-70">Terima kasih membantu warga lain, Lur!</span>
      `;
      document.body.appendChild(toastNotif);
  ...
  // Gabungkan hasil dari database dan nominatim
  const allResults = [
    ...tempatList.map(t => ({ ...t, source: 'database' })),
    ...nominatimResults
  ];
      ...
      <AnimatePresence>
        {status === "success" && createPortal(
          <motion.div
            ...
            className="fixed bottom-24 left-1/2 -translate-x-1/2 z-[1000002] px-6 py-4 rounded-3xl shadow-2xl flex flex-col items-center gap-1 min-w-[280px] border text-center bg-emerald-950 border-emerald-500/50 text-emerald-100"
          >
            <div className="text-2xl mb-1">✅</div>
            <span className="text-[12px] font-black uppercase tracking-widest">Laporan Terkirim!</span>
            <span className="text-[10px] opacity-70">Terima kasih membantu warga lain, Lur!</span>
          </motion.div>,
          document.body
        )}
      </AnimatePresence>
        ...
        <div className="flex flex-col flex-1 min-h-0">
              ...
              <div className="flex items-center justify-between">
                <label className="flex items-center gap-2 cursor-pointer">
                  ...
                  <span className="text-[10px] font-medium text-slate-500">
                    🔍 Cari juga di peta (jalan, simpang, lokasi umum)
                  </span>
                </label>
              </div>

              <div className="overflow-y-auto flex flex-col gap-2 pr-1 custom-scrollbar" style={{ maxHeight: "45vh" }}>
                {isLoadingTempat && tempatList.length === 0 && !isSearchingNominatim ? (
                ...
                ) : allResults.length === 0 && tempatQuery ? (
                  <div className="py-20 text-center">
                    <p className="text-[12px] text-slate-500">Tidak ditemukan</p>
                    {!showNominatim && (
                      <button
                        onClick={() => setShowNominatim(true)}
                        className="mt-2 text-[10px] text-emerald-500 underline"
                      >
                        Coba cari di peta →
                      </button>
                    )}
                  ...
                  allResults.map(t => (
                    ...
                    >
                      <div className="w-10 h-10 rounded-xl bg-white shadow-sm flex items-center justify-center text-lg group-hover:scale-110 transition-transform">
                        {t.source === 'nominatim' ? '🛣️' : '📍'}
                      </div>
                      <div className="min-w-0 flex-1">
                        ...
                        <p className="text-[10px] text-slate-500 truncate">
                          {t.source === 'nominatim' ? t.fullName : `${t.category} • ${t.alamat}`}
                        </p>
                      </div>
                    </button>
                  ))
            ...
            <div className={`flex-shrink-0 px-5 py-4 flex items-center justify-between border-b ${theme?.isMalam ? "border-white/5" : "border-slate-50"}`}>
              <div className="flex flex-col min-w-0">
                <div className="flex items-center gap-2">
                  ...
                  {isNomimatimLocation && (
                    <span className="text-[8px] px-1.5 py-0.5 rounded-full bg-blue-100 text-blue-600">Jalan</span>
                  )}
                </div>
                <div className="flex items-center gap-2">
                ...
                {({ open }) => (
                  ...
                  >
                    ...
                    <div className="text-center">
                      ...
                      <p className="text-[10px] text-slate-400">Bukti kondisi di lokasi</p>
                    </div>
                  </button>
                )}
              </CldUploadWidget>
          ...
          {step === "form" && (
            <div className="flex-1 overflow-y-auto px-5 py-4 space-y-5 pr-2 custom-scrollbar">
              ...
              {/* Kondisi Lalu Lintas (untuk tempat dengan kategori jalan ATAU lokasi dari Nominatim) */}
              {(hasTraffic || isNomimatimLocation) && (
                <div className="pt-2 border-t border-slate-50 space-y-3">
                  <label className="text-[10px] font-black text-slate-400 uppercase tracking-wider ml-1">Kondisi Lalu Lintas</label>
                  <div className="grid grid-cols-3 gap-2">
                    {[
          ...
          {(step === "form" || step === "upload") && (
            <div className="p-5 border-t border-slate-100 bg-white/50 backdrop-blur-sm">
              <button
                ...
                disabled={
                  (isNomimatimLocation && !trafficCondition) ||
                  (!isNomimatimLocation && !condition && !trafficCondition) ||
                  (condition === "Antri" && !waitTime) ||
                  status === "uploading"
                }
                className={`w-full py-4 rounded-2xl font-black uppercase text-[12px] tracking-[0.2em] transition-all flex items-center justify-center gap-3 shadow-xl ${((isNomimatimLocation && !trafficCondition) ||
                    (!isNomimatimLocation && !condition && !trafficCondition) ||
                    (condition === "Antri" && !waitTime) ||
                    status === "uploading")
              ...
              >
                {status === "uploading" ? (
                  ...
                  <>Kirim Laporan {mediaUrl ? "dengan Media" : "Cepat"}</>
                )}
              </button>
            </div>
          )}
