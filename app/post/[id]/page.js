...
import HeroCard from "@/components/v2/HeroCard"; // ✅ PAKAI HERO CARD
  ...
  return (
    <div className={`relative min-h-screen w-full ${theme.bg} ${theme.text} transition-colors duration-500`}>
      ...
      <div className="relative z-10">
        ...
        <main ref={mainContentRef} className="mx-auto w-full max-w-[420px] px-2 pt-20 pb-36">
          <LayoutGroup>
            ...
            {/* 1. HERO CARD & AI INSIGHT (DIPISAHKAN DENGAN RAPI) */}
            <div ref={heroRef} className="relative mb-2">
              <LayoutGroup>
                <motion.div layout transition={{ type: "spring", stiffness: 300, damping: 30 }}>

                  {/* Container Foto (Bersih, tidak tertutup) */}
                  ...
                  {/* AI INSIGHT (Sekarang berada di bawah foto, bukan lagi 'absolute' yang menutupi) */}
            ...
            <div className="flex items-center justify-between mt-6 mb-4 px-2">
              ...
              <div className="flex items-center gap-3 bg-black/5 dark:bg-white/5 p-1.5 rounded-2xl border border-black/5 dark:border-white/5">
                <button onClick={scrollToTop} className={`px-4 py-1.5 rounded-xl transition-all duration-300 text-[9px] font-black uppercase ${activeSection === 'profil' ? 'bg-white dark:bg-zinc-800 shadow-md opacity-100' : 'opacity-30 hover:opacity-60'}`}>Profil</button>
                <button onClick={scrollToRadar} className={`px-4 py-1.5 rounded-xl transition-all duration-300 text-[9px] font-black uppercase ${activeSection === 'radar' ? 'bg-white dark:bg-zinc-800 shadow-md opacity-100' : 'opacity-30 hover:opacity-60'}`}>Radar</button>
              </div>
            </div>

            {/* Radar Section */}
            <motion.section ref={radarRef} initial={{ opacity: 0, y: 20 }} animate={{ opacity: 1, y: 0 }} transition={{ duration: 0.4 }} className={`p-6 rounded-[40px] border ${isMalam ? 'bg-zinc-950/50 border-white/10' : 'bg-white border-black/5'} backdrop-blur-3xl shadow-2xl relative overflow-hidden`}>
      ...
      <div className="fixed bottom-24 right-6 z-[99] flex flex-col items-end gap-3.5 select-none">
        <AnimatePresence>
          {isExpanded && (
            <motion.div initial={{ opacity: 0, y: 15, scale: 0.85 }} animate={{ opacity: 1, y: 0, scale: 1 }} exit={{ opacity: 0, y: 15, scale: 0.85 }} transition={{ duration: 0.2 }} className="flex flex-col gap-3 mb-1">
              <button onClick={() => { handleOpenAIModal(`Halo! Tolong ringkas kondisi ${tempatName} untuk kategori ${kategori}`); setIsExpanded(false); }} className="flex items-center gap-3 bg-gradient-to-r from-amber-600 to-orange-600 text-white px-4 py-2.5 rounded-xl shadow-xl border border-white/10 active:scale-95 transition-all">
              ...
              <button onClick={() => { window.open(`https://wa.me/${adminPhone}?text=Halo Admin ${tempatName}...`); setIsExpanded(false); }} className="flex items-center gap-3 bg-gradient-to-r from-emerald-600 to-teal-600 text-white px-4 py-2.5 rounded-xl shadow-xl border border-white/10 active:scale-95 transition-all">
                <div className="flex flex-col items-end">
        ...
        <motion.button onClick={() => setIsExpanded(!isExpanded)} whileTap={{ scale: 0.92 }} className={`w-16 h-16 rounded-full flex items-center justify-center shadow-2xl border-2 transition-all duration-300 relative ${isExpanded ? 'bg-zinc-900 border-zinc-800' : 'bg-white dark:bg-zinc-900 border-zinc-100 dark:border-zinc-800'}`}>
      ...
      <AnimatePresence>
        {toast.show && (
          <motion.div initial={{ y: 50, opacity: 0 }} animate={{ y: 0, opacity: 1 }} exit={{ y: 50, opacity: 0 }} className="fixed bottom-28 left-1/2 -translate-x-1/2 z-[150] px-6 py-3 rounded-full bg-zinc-900 border border-white/20 text-white text-[10px] font-bold">
            {toast.message}
          </motion.div>
        )}
      </AnimatePresence>
