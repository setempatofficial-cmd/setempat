...
export function calculateScore(item, location) {
  ...
  activityScore += Math.min(30, komentarCount * 5);
  
  const laporanCount = item.laporan_terbaru?.length || 0;
  activityScore += Math.min(30, laporanCount * 10);
  
  const estimasiOrang = parseInt(item.estimasi_orang) || 0;
  activityScore += Math.min(30, estimasiOrang * 0.5);
  ...
  if (item.lastActivity) {
    ...
    if (hoursAgo < 1) {
      ...
      freshnessScore = 80 - (hoursAgo - 1) * 2;
    } else if (hoursAgo < 72) {
  ...
  // 5. TRUST (0–50 poin) - bobot 5% (GABUNGKAN DI SINI)
  let trustScore = 10; // Default
  
  // Tambahkan berdasarkan matching confidence (jika ada)
  ...
  // Tambahkan jika ada external signals dengan confidence tinggi
